...
EXCLUDEFILE="/source/AppDev-ContainerDemo/environment/reset-demo-template-values.sh"

sudo grep -rl REPLACE-JSON-SUBSCRIPTIONID /source/AppDev-ContainerDemo --exclude {$EXCLUDEFILE}  | sudo xargs sed -i -e "s@REPLACE-JSON-SUBSCRIPTIONID@$AZJSONSUBSCRIPTIONID@g"
...
sudo grep -rl VALUEOF-UNIQUE-SERVER-PREFIX /source/AppDev-ContainerDemo --exclude {$EXCLUDEFILE}  | sudo xargs sed -i -e "s@VALUEOF-DEMO-ADMIN-USER-NAME@$DEMO_ADMIN_USER@g"
